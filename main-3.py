...
def print_all_commands(call): 
  ...
  if call.data == "trek":
    ...
    k4 = types.InlineKeyboardButton(text="EXCEL",callback_data="ex") 
    k5 = types.InlineKeyboardButton(text="НАЗАД",callback_data="back") 
    kb_change_trak.add(k1,k2,k3,k4,k5)
  ...
  if call.data == "ex":
    variable.set_action(call.message.chat.id, 6)
    kb_change_trak = types.InlineKeyboardMarkup(row_width=1)
    k3 = types.InlineKeyboardButton(text="НАЗАД",callback_data="back") 
    kb_change_trak.add(k3)
    bot.send_message(call.message.chat.id, "ОТПРАВЬТЕ EXCEL ФАИЛ ФОРМАТА\nТРЕК Статус",reply_markup=kb_change_trak)
...
def handle_text(message):
  ...
  if action == 6: 
    try:
        ...
        k3 = types.InlineKeyboardButton(text="НАЗАД", callback_data="back")
        kb_change_trak.add(k3)
        bot.send_message(message.chat.id, "Успешно добавлены трек номера", reply_markup=kb_change_trak)
    except Exception as e:
